...
export const getServerSideProps = async (ctx: any) => {
  const isLoggedIn: string | null | undefined = parseCookies(ctx.req).token;
...
const SingleAssignment = ({ data }: Props) => {
  ...
  const deleteAssignment = async (e) => {
    e.preventDefault();
    setSubmitting(true);
    const url: string = `${process.env.NEXT_PUBLIC_API_URI}/assignments/${router.query.id}`;
    const options: {
      method: string;
      headers: { 'Content-Type': string; authorization: string };
    } = {
  ...
  if (data) {
    const dueDate: Date = new Date(data.message.dueDate);
    formattedDate =
      dueDate.getDate() +
      '/' +
      (dueDate.getMonth() + 1) +
      '/' +
      dueDate.getFullYear();
    const createdAt: Date = new Date(data.message.createdAt);
    formattedCreatedAt =
      createdAt.getDate() +
      '/' +
